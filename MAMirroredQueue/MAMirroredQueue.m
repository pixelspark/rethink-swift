...
- (void)advanceReadPointer: (size_t)howmuch
{
    ...
    if((size_t)(_readPointer - _buf) >= _bufSize)
    {
        ...
        __sync_sub_and_fetch(&_writePointer, (char*)_bufSize);
    }
}
...
{
    __sync_add_and_fetch(&_writePointer, (char*)howmuch);
}
        ...
        @autoreleasepool
        {
            ...
            dispatch_group_async(group, dispatch_get_global_queue(0, 0), ^{
                while([inData length] < targetLength)
                    @autoreleasepool
                    {
                        unsigned long len = nrand48(seedPtr1) % 1024 + 1;
                        unsigned long remaining = targetLength - [inData length];
            ...
            dispatch_group_async(group, dispatch_get_global_queue(0, 0), ^{
                while([outData length] < targetLength)
                    @autoreleasepool
                    {
                        unsigned long len = nrand48(seedPtr2) % 10240 + 1;
                        [queueLock lock];
                        unsigned long available = [queue availableBytes];
                        [queueLock unlock];
